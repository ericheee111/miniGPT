...
- `PublicDemoPolicy.streaming_enabled` defaults to false in code. The canonical deployment config is true only because the 2026-08-31 real Funnel acceptance proved separate chunks, cancellation, and slot/KV cleanup; the frontend still must not request SSE until `/demo/info` returns true.

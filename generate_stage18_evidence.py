"""Generate the Stage 18 lazy KV-reservation evidence package."""
...
def build_parser() -> argparse.ArgumentParser:
    """Create the Stage 18 evidence-generation parser."""
    parser = argparse.ArgumentParser(description="Generate hash-bound Stage 18 evidence.")
    _ = parser.add_argument("--source-commit", required=True)
    _ = parser.add_argument(
        "--package-root",
        type=Path,
        default=Path("docs/results/lazy-kv-reservation"),
    )
    _ = parser.add_argument("--work-root", type=Path, default=Path("reports/stage18-evidence"))
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    correctness = generate_stage18_correctness(work_root / "correctness.json")
    scheduling = generate_stage18_scheduling(work_root / "scheduling.json")
    apc = generate_stage18_apc(work_root / "apc.json")
    stress = write_stage18_stress(work_root / "stress.json", operations=1000)
    ...
    lifecycle = _test_evidence(
        ...
        (
            "tests/test_lazy_kv_allocator.py",
            "tests/test_lazy_kv_reservation.py",
            "tests/test_lazy_kv_reservation_simulator.py",

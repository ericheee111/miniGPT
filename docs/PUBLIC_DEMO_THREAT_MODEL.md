...
默认拓扑：GitHub Pages → ngrok assigned development domain → `127.0.0.1:8000` → `minigpt demo-serve`。
适用范围：本仓库实现的静态客户端、public-demo HTTP boundary、launcher 和部署 workflow。

## 1. 安全目标

1. checkpoint、tokenizer、ngrok authtoken、环境 secret 和本机路径始终留在本地；
...
| 资产 | 位置 | 保护要求 |
|---|---|---|
| ngrok authtoken | ngrok 用户配置 | secret；不进入 repo、环境示例、参数、日志或 Pages |
...
ngrok 终止公网 HTTPS 并转发到 loopback。它是第三方 trust boundary，可能有自身 traffic inspection、events、quota 和账户日志。用户必须假定 tunnel provider 能观察转发 metadata，且服务额度/策略可能变化。不要输入敏感内容。

### 4.3 Loopback public-demo HTTP boundary

`minigpt demo-serve` 默认只监听 `127.0.0.1`。非 loopback bind 需要命名明确的 `--unsafe-allow-non-loopback`；`trust_proxy` 与非 loopback bind 不能同时使用。Uvicorn access log、Swagger、Redoc、OpenAPI、proxy header rewriting、server/date identification headers均关闭。
...
checkpoint/tokenizer 由显式本地路径读取。launcher 不加载未审计 `.env`，不接收 authtoken，不修改 firewall，不创建 port forwarding。日志位于 gitignored `outputs/public-demo/`，只应含 process output 和 request ID，不含 Prompt。
...
| 伪造 `X-Forwarded-For` | 默认 `trust_proxy=false`；启用时强制 loopback bind；始终存在独立 global limiter | trusted proxy chain 配置错误会降低 per-client 精度 |
| 通过轮换 XFF 绕过配额 | global sliding-window limiter 不读取 client key，所有请求共享 | limiter 为单进程内存状态，重启会清零 |
...
| queue/rate race | `asyncio.Lock` 原子更新 gate 与 limiter；FIFO waiter transfer | 多进程扩展不共享状态，本轮不支持多实例 |
...
| Prompt 进入日志 | Uvicorn access log off；应用日志仅 request ID 与聚合终态；错误不记录 exception text | ngrok/OS/外部诊断工具有独立数据策略 |
...
| ngrok token 进入 Git | launcher 无 token parameter；`.env*`/`ngrok.yml` ignored；示例不含 secret；部署文档要求用户配置 | 操作者手工误提交需 secret scanning/review 发现 |
...
- 允许 `GET`、`POST`、`OPTIONS`；preflight 只接受请求所需的 `content-type` 和 `ngrok-skip-browser-warning`；
...
## 7. Client IP 与 proxy trust

默认 `trust_proxy=false`，per-client limiter 使用 ASGI peer。在 ngrok loopback topology 中，这可能把多个公网访问者视为同一个 proxy peer，结果是更保守的共享 per-client bucket；global bucket仍始终生效。

只有 operator 明确启用 `--trust-proxy` 或 typed policy 时才解析 `X-Forwarded-For`。此模式要求 backend 继续监听 loopback，并把最右侧地址视为 trusted proxy 添加的直接 client hop。启用前必须验证实际 tunnel 的 header append/overwrite 行为；若不能验证，保持关闭。无论该 header 如何，global limiter 都不使用它，所以伪造 client key 不能绕过 global quota。

本设计不返回、记录或在 `/demo/metrics` 聚合 client IP。
...
- ngrok 是独立数据处理边界，可能保留 provider-side events；用户提示必须说“不要输入敏感信息”，不能承诺 end-to-end zero retention。
...
3. tunnel/process 层：Ctrl+C launcher，停止精确 child PID；必要时在 ngrok dashboard 停 endpoint/撤销 token。

backend offline 不影响 GitHub Pages。没有自动启动 ngrok service、管理员权限、firewall rule 或 router mapping。
...
Swagger、Redoc 和 public OpenAPI 均禁用。`/demo/info` 只返回 version、model id、mode、public limits、executor、KV backend、prefix-cache state 和 streaming availability。`/demo/metrics` 只返回 online、active/queued/completed/failed/rejected/rate-limited/timeout、generated-token count 与 average latency。
...
4. per-client + global rate limits及 `Retry-After`；
5. queue full；
...
12. no `innerHTML`、custom ngrok header、AbortController、hidden-tab polling；

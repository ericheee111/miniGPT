"""Run the Stage 14 Automatic Prefix Caching fresh-process benchmark."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Benchmark Automatic Prefix Caching on CPU.")
    _ = parser.add_argument("--output", type=Path, required=True)
    _ = parser.add_argument("--warmups", type=int, default=0)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    path = write_prefix_cache_benchmark(
        cast("Path", arguments.output),
        config=PrefixCacheBenchmarkConfig(

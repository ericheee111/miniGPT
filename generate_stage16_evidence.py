"""Generate the Stage 16 chunked-prefill and token-budget evidence package."""
...
def build_parser() -> argparse.ArgumentParser:
    """Create the Stage 16 evidence-generation parser."""
    parser = argparse.ArgumentParser(description="Generate hash-bound Stage 16 evidence.")
    _ = parser.add_argument("--source-commit", required=True)
    _ = parser.add_argument(
        "--package-root",
        type=Path,
        default=Path("docs/results/chunked-prefill-token-budget"),
    )
    _ = parser.add_argument("--work-root", type=Path, default=Path("reports/stage16-evidence"))
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    correctness = generate_stage16_correctness(work_root / "correctness.json")
    scheduling = generate_stage16_scheduling(work_root / "scheduling.json")
    ...
    stress = write_stage16_stress(work_root / "stress.json", operations=1000)
